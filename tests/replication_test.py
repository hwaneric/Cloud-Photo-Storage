...
@patch("driver.grpc.channel_ready_future")
@patch("driver.server_pb2_grpc.ServerStub")
def test_persistent_storage_per_server_instance(mock_stub, mock_ready, mock_channel):
    # Mock gRPC behavior to avoid real connections
    mock_ready.return_value.result.return_value = None
    mock_channel.return_value = MagicMock()
    mock_stub.return_value = MagicMock()

    # Create a temporary directory for server 0's persistent storage
    with tempfile.TemporaryDirectory() as server0_db:
        # Create necessary subdirectories
        os.makedirs(os.path.join(server0_db, "unread_messages"))
        os.makedirs(os.path.join(server0_db, "sent_messages"))

        # First server instance (simulate account creation)
        s0 = Server(id=0, host="localhost", port="5000", db_path=server0_db)
        connect(s0)

        signup_req = server_pb2.UserAuthRequest(username="alice", password="123", from_client=True)
        s0.is_leader = True
        signup_res = s0.Signup(signup_req, context=None)
        assert signup_res.success

        logout_req = server_pb2.UserLogoutRequest(username="alice")
        logout_res = s0.Logout(logout_req, context=None)
        assert logout_res.success

        # Simulate server crash/shutdown
        del s0

        # Restart server 0 using the same db path
        s0_rebooted = Server(id=0, host="localhost", port="5000", db_path=server0_db)
        connect(s0_rebooted)
        s0_rebooted.is_leader = True

        # Login again to confirm persistence
        login_req = server_pb2.UserAuthRequest(username="alice", password="123", from_client=True)
        login_res = s0_rebooted.Login(login_req, context=None)
        assert login_res.WhichOneof("response") == "success"
        assert login_res.success.success
        assert login_res.success.unread_message_count == 0

        # Confirm persistence files exist
        assert os.path.exists(os.path.join(server0_db, "user_data.json"))
        assert os.path.exists(os.path.join(server0_db, "unread_messages", "alice.json"))